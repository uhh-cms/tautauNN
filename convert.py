...
dtype = [(v, np.uint) if v in ["EventNumber"] else (v, np.float32) for v in variables]
nentries_total = {}
nentries_selected = {}
...
def root2npz(filename):
    ...
    outputfilename = outputfilename.replace("riegerma", "kramerto").replace("02Mar", "17Jul")
    ...
    np.savez(outputfilename, events=r, weightsum=weightsum)
...
for dirname in dirnames:
    if ".txt" in dirname or ".sh" in dirname:
        continue
    Path(dirname.replace("riegerma", "kramerto").replace("02Mar", "17Jul")).mkdir(parents=True, exist_ok=True)

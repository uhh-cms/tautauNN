...
from util import load_data, phi_mpi_to_pi, split_train_validation_mask, calc_new_columns, create_tensorboard_callbacks
...
def main(basepath                   =   "/nfs/dust/cms/user/kramerto/hbt_resonant_run2/HHSkims/SKIMS_uhh_2017_v2_31Aug22",
         ...
         samples                    =   {
                                        "SKIM_GGHH_SM": 1.0,
                                        # "SKIM_ggF_Radion_m300": 1.0/34,
                                        # "SKIM_ggF_Radion_m350": 1.0/34,
                                        # "SKIM_ggF_Radion_m400": 1.0/34,
                                        # "SKIM_ggF_Radion_m450": 1.0/34,
                                        # "SKIM_ggF_Radion_m500": 1.0/34,
                                        # "SKIM_ggF_Radion_m550": 1.0/34,
                                        # "SKIM_ggF_Radion_m600": 1.0/34,
                                        # "SKIM_ggF_Radion_m650": 1.0/34,
                                        # "SKIM_ggF_Radion_m700": 1.0/34,
                                        # "SKIM_ggF_Radion_m750": 1.0/34,
                                        # "SKIM_ggF_Radion_m800": 1.0/34,
                                        # "SKIM_ggF_Radion_m850": 1.0/34,
                                        # "SKIM_ggF_Radion_m900": 1.0/34,
                                        # "SKIM_ggF_Radion_m1000": 1.0/34,
                                        # "SKIM_ggF_Radion_m1250": 1.0/34,
                                        # "SKIM_ggF_Radion_m1500": 1.0/34,
                                        # "SKIM_ggF_Radion_m1750": 1.0/34,
                                        # "SKIM_ggF_BulkGraviton_m300": 1.0/34,
                                        # "SKIM_ggF_BulkGraviton_m350": 1.0/34,
                                        # "SKIM_ggF_BulkGraviton_m400": 1.0/34,
                                        # "SKIM_ggF_BulkGraviton_m450": 1.0/34,
                                        # "SKIM_ggF_BulkGraviton_m500": 1.0/34,
                                        # "SKIM_ggF_BulkGraviton_m550": 1.0/34,
                                        # "SKIM_ggF_BulkGraviton_m600": 1.0/34,
                                        # "SKIM_ggF_BulkGraviton_m650": 1.0/34,
                                        # "SKIM_ggF_BulkGraviton_m700": 1.0/34,
                                        # "SKIM_ggF_BulkGraviton_m750": 1.0/34,
                                        # "SKIM_ggF_BulkGraviton_m800": 1.0/34,
                                        # "SKIM_ggF_BulkGraviton_m850": 1.0/34,
                                        # "SKIM_ggF_BulkGraviton_m900": 1.0/34,
                                        # "SKIM_ggF_BulkGraviton_m1000": 1.0/34,
                                        # "SKIM_ggF_BulkGraviton_m1250": 1.0/34,
                                        # "SKIM_ggF_BulkGraviton_m1500": 1.0/34,
                                        # "SKIM_ggF_BulkGraviton_m1750": 1.0/34,
                                        # "SKIM_DYmerged": 1.0,                                                                                                                                                        
    ...
    data, eventweights = load_data(basepath, samples, columns_to_read, selections)

    data = calc_new_columns(data, columns_to_add)
    
    float_input_vecs = data[float_inputs]
    int_input_vecs = data[int_inputs]
    target_vecs = data[targets]
    ...
    target_stds = None
    if output_scaling:
        target_means = np.mean(target_vecs, axis = 0)
        target_stds = np.std(target_vecs, axis = 0)
        target_means = np.array(target_means)
        target_stds = np.array(target_stds)
        target_vecs = (target_vecs - target_means)/target_stds
    
    train_mask = split_train_validation_mask(len(eventweights), fraction = train_valid_fraction, seed = train_valid_seed)
    valid_mask = ~train_mask

    float_input_train, float_input_valid = float_input_vecs[train_mask], float_input_vecs[valid_mask]
    int_input_train, int_input_valid = int_input_vecs[train_mask], int_input_vecs[valid_mask]
    target_train, target_valid = target_vecs[train_mask], target_vecs[valid_mask]
    eventweights_train, eventweights_valid = eventweights[train_mask], eventweights[valid_mask]
    
    del float_input_vecs, int_input_vecs, eventweights
        
    dataset_train = create_dataset(float_input_train, int_input_train, target_train, eventweights_train, shuffle=True, repeat=epochs, batch_size=batch_size, seed=None)
    dataset_valid = create_dataset(float_input_valid, int_input_valid, target_valid, eventweights_valid, shuffle=False, repeat=1, batch_size=-1, seed=None)
    
    float_input_shape, int_input_shape, output_shape = float_input_train.shape[1], int_input_train.shape[1], target_train.shape[1]
    nevents_train = float_input_train.shape[0]
    
    del float_input_train, float_input_valid, int_input_train, int_input_valid, target_train, target_valid, eventweights_train, eventweights_valid
    
    with device:
        model, regularization_weights = create_model(float_input_shape = float_input_shape,
                                                     int_input_shape = int_input_shape,
                                                     output_shape = output_shape,
                                                     embedding_expected_inputs = embedding_expected_inputs, 
        ...
        best_weights, _ = training_loop(dataset_train,
                                            ...
                                            optimizer,
                                            learning_rate,
                                            max_steps=int(math.ceil(nevents_train/batch_size * epochs)),
                                            log_every=10,
                                            validate_every=100,
...
def create_dataset(float_input_vecs, int_input_vecs, target_vecs, eventweights, shuffle=False, repeat=1, batch_size=100, seed=None, **kwargs):
    nevents =  float_input_vecs.shape[0]
    
    # create a tf dataset
    data = (float_input_vecs, int_input_vecs, target_vecs, np.expand_dims(eventweights, axis=1))
...
def create_model(float_input_shape, int_input_shape, output_shape, embedding_expected_inputs, embedding_output_dim, target_means = None, target_stds = None, units=(128, 128, 128), activation="selu", dropout_rate=0.):
    ...
    if int_input_shape > 0:
        ...
        a = tf.keras.layers.Concatenate()([x1, a])
    else:
        a = x1    
        
    a = tf.keras.layers.BatchNormalization(dtype="float32")(a)
...
def create_losses(modelweights, l2_norm=10):
    ...
    # cross entropy
    @tf.function
    def loss_ce_fn(labels, predictions, eventweights):
        # ensure proper prediction values before applying log's
        predictions = tf.clip_by_value(predictions, 1e-6, 1 - 1e-6)
        loss_ce = tf.reduce_mean(eventweights * -labels * tf.math.log(predictions))
        return loss_ce

    # l2 loss
    @tf.function
    def loss_l2_fn(labels, predictions, eventweights):
    ...
    # MSE loss
    @tf.function
    def loss_mse_fn(labels, predictions, eventweights):
        # compute the mse loss
        loss_mse = tf.reduce_mean(eventweights * (labels - predictions)**2.)
        return loss_mse
...
def training_loop(
    ...
    def update_metrics(bar, kind, step, targets, predictions, losses, total_loss):
    ...
    with tqdm(total=max_steps, bar_format=fmt, postfix=[metrics]) as bar:
        for step, (float_inputs, int_inputs, targets, eventweights) in enumerate(dataset_train):
            ...
            if do_validate:
                float_inputs_valid, int_inputs_valid, targets_valid, eventweights_valid = next(iter(dataset_valid))
                ...
                is_best = update_metrics(bar, "valid", step, targets_valid, predictions_valid, losses_valid, total_loss_valid)

...
class LBNInputSelection(tf.keras.layers.Layer):

    def __init__(self, feature_indices, name="lbn_input_selection", **kwargs):
        super().__init__(name=name, **kwargs)

        assert len(feature_indices) % 4 == 0, "feature indices must be divisible by 4"
        self.feature_indices = feature_indices
        self.n_input_particles = len(self.feature_indices) // 4

    @property
    def lbn_input_shape(self):
        return (self.n_input_particles, 4)

    def get_config(self):
        config = super().get_config()
        config.update({"feature_indices": self.feature_indices})
        return config

    def call(self, inputs):
        if all(i >= 0 for i in self.feature_indices):
            flat_inputs = tf.gather(inputs, self.feature_indices, axis=1)
        else:
            zeros = tf.zeros_like(inputs[:, 0])
            flat_inputs = [
                inputs[:, i] if i >= 0 else zeros
                for i in self.feature_indices
            ]
            # set missing energies to absolute momenta
            for i in range(0, len(self.feature_indices), 4):
                if self.feature_indices[i] >= 0:
                    continue
                p2 = flat_inputs[i + 1]**2 + flat_inputs[i + 2]**2 + flat_inputs[i + 3]**2
                flat_inputs[i] = (p2 + 1e-5) ** 0.5
            # concat
            flat_inputs = tf.stack(flat_inputs, axis=1)

        return tf.reshape(flat_inputs, (-1, self.n_input_particles, 4))
